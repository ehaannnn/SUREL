...
class Yelp():
    def __init__(self):
        ...
        self.split_edge = {'train': {'edge': torch.Tensor(user2item_edge_index_train[:,126420:]).type(torch.int32)},
        'valid': {'edge': torch.Tensor(user2item_edge_index_train[:,:126420]).type(torch.int32),
                 'edge_neg': torch.Tensor(user2item_edge_index_train[:,:63210]).type(torch.int32)},
        'test': {'edge': torch.Tensor(user2item_edge_index_test[:,:23390]).type(torch.int32),
                'edge_neg': torch.Tensor(user2item_edge_index_test[:,23390:]).type(torch.int32)}}
